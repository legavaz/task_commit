...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОбновлениеКонфигурации") Тогда
		...
		Если СтандартныеПодсистемыСервер.ЭтоБазоваяВерсияКонфигурации()
			...
			Элементы.УстановкаОбновлений.РасширеннаяПодсказка.Заголовок = НСтр(
				"ru = 'Обновление программы из файла на локальном диске или в сетевой папке.'");
		КонецЕсли;
	Иначе
...
Процедура ОткрытьСписокФайлов(Команда)    
	...
	ПараметрыФормы = Новый Структура("Ошибка",Истина);
	
	ОткрытьФорму("РегистрСведений.ЗД_ЗагруженныеФайлыЗамеров.ФормаСписка",ПараметрыФормы,ЭтаФорма);
	
КонецПроцедуры

&НаСервере
Процедура ЗагрузитьНаСервере()
	ОценкаПроизводительности.ЗагрузитьФайлы_apdex();
КонецПроцедуры
