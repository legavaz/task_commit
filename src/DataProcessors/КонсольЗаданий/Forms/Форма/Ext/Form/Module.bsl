...
&НаКлиенте
Процедура ПриОткрытии(Отказ) //sdfsdfsdf
	Попытка
...
Процедура ОтменитьФоновыеЗадания()
	...
	Для Каждого Стр из ВыделенныеСтроки Цикл
		...
		УИД_ФЗД = Новый УникальныйИдентификатор(Строка.Идентификатор);
		ФоновоеЗадание = ФоновыеЗадания.НайтиПоУникальномуИдентификатору(УИД_ФЗД);
...
&НаСервере
Процедура ВыполнитьЗаданиеНаСервере(ЗначениеИдентификатора)
	
	ТекущийИдентификатор = Новый УникальныйИдентификатор(ЗначениеИдентификатора);
	мРегламентноеЗадание = РегламентныеЗадания.НайтиПоУникальномуИдентификатору(ТекущийИдентификатор);
	
	ИмяМетода    = мРегламентноеЗадание.Метаданные.ИмяМетода;
	ПараметрыЗадания    = мРегламентноеЗадание.Параметры;
	Ключ         = мРегламентноеЗадание.Ключ;
	Наименование = мРегламентноеЗадание.Наименование;   	
	
	Попытка
		
		Выполнить(ИмяМетода+"()");
		Сообщить("Выполнено "+ТекущаяДата()+ Символы.ПС+ ИмяМетода);
