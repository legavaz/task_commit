...
Процедура ВыполнитьКонвертацию(Знач Хранилище = Неопределено) Экспорт
	...
	Если Параметры.ПоследняяВерсия = 0 Тогда
		...
		СтрокаСоединения = "/S srv1cdev.office.local:1541\empty2"; //Данилин 
...
Процедура ВыполнитьКоммиты(Знач Хранилище) Экспорт
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ЗапуститьПриложение(?(ЭтоWindowsСервер, "cmd /C ", "bash -c ") + ТекстКоманды, 
...
//<< Danilin.V 04.03.2025 
//ИБ
//Рабочий каталог
Функция СФормироватьПараметрыИВыгрузитьВерсии(ИБ,РабочийКаталог,ВыгружатьПолностью=Истина) Экспорт	
	

	мРабочийКаталог = АвтономныйСервер.глПолучитьКаталог(РабочийКаталог);	
	
	Если не АвтономныйСервер.ПроверитьНаличиеФайла(мРабочийКаталог) Тогда 
		СоздатьКаталог(мРабочийКаталог);	
	КонецЕсли;  
	
	РезультатСтруктура = новый Структура("Хранилище,ИмяФайлаОтчета"); //для исследования ошибок
	
	ПараметрыСтруктура = Новый Структура("ИмяФайлаЛогов,ИмяФайлаРезультатов,ИмяФайлаОтчета,
	|ВерсияНачала,ВерсияОкончания, 
	|КаталогИсполняемогоФайла,ФайлПараметровПакетнойОперации,
	|СтрокаСоединенияИБ,ИмяПользователяИБ,ПарольПользователяИБ,
	|АдресХранилища,ИмяПользователяХранилища,ПарольПользователяХранилища,ЭтоРасширение,Наименование");	
	...
	ПараметрыСтруктура.КаталогИсполняемогоФайла = СтрЗаменить(Версия1С,"1cv8.exe","\");  
		
	РеквизитыИБ = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ИБ,"СтрокаПодключенияБазы,ИмяБазы,ЛогинБазы,ПарольБазы,Хранилище");	

	ПараметрыСтруктура.СтрокаСоединенияИБ = РеквизитыИБ.СтрокаПодключенияБазы; 
	ПараметрыСтруктура.ИмяПользователяИБ = РеквизитыИБ.ЛогинБазы; 	
	ПараметрыСтруктура.ПарольПользователяИБ = РеквизитыИБ.ПарольБазы; 
	        
	ХранилищаИБ = РеквизитыИБ.Хранилище.Выгрузить();
	
	Для каждого ТекХранилища из ХранилищаИБ Цикл  
		
		Если не ЗначениеЗаполнено(ТекХранилища.Хранилище) Тогда
			ОбщегоНазначения.СообщитьПользователю("в списке хранилищ пропущена незаполненная строка: "+ТекХранилища.номерСтроки);
			Продолжить;
		КонецЕсли;
				
		РеквизитыХран = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ТекХранилища.Хранилище,"Наименование,ПутьХранилища,ЭтоРасширение");
		
		ИмяХранилища = УбратьЛишниеСимволы(РеквизитыХран.Наименование);
		Если ПустаяСтрока(ИмяХранилища) Тогда
			ИмяХранилища = "rep_"+ТекХранилища.номерСтроки;
		КонецЕсли;		
		
		ПараметрыСтруктура.ФайлПараметровПакетнойОперации = мРабочийКаталог	+ "params_" + "key"+ ".txt";
		ПараметрыСтруктура.ИмяФайлаЛогов 		= мРабочийКаталог+ИмяХранилища+"_log.txt";
		ПараметрыСтруктура.ИмяФайлаОтчета		= мРабочийКаталог+ИмяХранилища+"_report.mxl";
		ПараметрыСтруктура.ИмяФайлаРезультатов	= мРабочийКаталог+ИмяХранилища+"_result.txt";   
		
				
		ПараметрыСтруктура.АдресХранилища = РеквизитыХран.ПутьХранилища; 
		ПараметрыСтруктура.ИмяПользователяХранилища = ТекХранилища.ЛогинХранилища; 	
		ПараметрыСтруктура.ПарольПользователяХранилища = ТекХранилища.ПарольХранилища; 
		
		ПараметрыСтруктура.ЭтоРасширение = РеквизитыХран.ЭтоРасширение; 
		ПараметрыСтруктура.Наименование = РеквизитыХран.Наименование; 

		
		КонвертацияХранилища.СформироватьОтчетПоВерсиямХранилища(ПараметрыСтруктура); 
		
		РезультатСтруктура.ИмяФайлаОтчета =	ПараметрыСтруктура.ИмяФайлаОтчета;
		РезультатСтруктура.Хранилище = ТекХранилища.Хранилище;
		
		Справочники.ВерсииХранилища.ЗагрузитьВерсииИзОтчета(ТекХранилища.Хранилище, ПараметрыСтруктура.ИмяФайлаОтчета);
...
Процедура СформироватьОтчетПоВерсиямХранилища(Параметры) Экспорт
	...
	//		И Параметры.СформироватьОтчетПоВерсиямХранилища Тогда
		...
		СтрокаПараметров = "DESIGNER /WA- /DisableStartupDialogs /L ru /VL ru
			...
			| " + ?(Параметры.ЭтоРасширение, "-Extension ""%ИмяРасширения%"" ", "") +"
			| /DumpResult ""%ИмяФайлаРезультатов%""
			| /Out ""%ИмяФайлаЛогов%"" -NoTruncate ";
		...
		СтрокаПараметров = СтрЗаменить(СтрокаПараметров, "%ИмяРасширения%", Параметры.Наименование);  
		...
		Если КодВозврата <> 0 ИЛИ КодРезультата <> 0 Тогда
			ОбщегоНазначения.СообщитьПользователю( СтрШаблон(НСтр("ru = 'При формирования отчета по хранилищу конфигураций возникли ошибки. Подробнее см. файл лога:
											 |%1'", Параметры.ИмяФайлаЛогов)));
		КонецЕсли;
