...
Процедура ЗагрузитьФайлы_apdex() Экспорт
	
	КаталогЗагрузки = Константы.ЗД_ПапкаФайлов_apdex.Получить();
	Если ПустаяСтрока(КаталогЗагрузки) Тогда
		ЗаписьЖурналаРегистрации("apdex.загрузка.ошибка",УровеньЖурналаРегистрации.Ошибка,Метаданные.Константы.ЗД_ПапкаФайлов_apdex,
		," не заполнен каталог загрузки файлов апдекс");
		Возврат;
	КонецЕсли; 
	
	МассивФайловАрхива = Новый Массив;
	
	Для Каждого Файл Из НайтиФайлы(КаталогЗагрузки, "*.XML") Цикл
		...
		Ошибка = Ложь;
		СтруктураОтвета = РегистрыСведений.ЗД_ЗагруженныеФайлыЗамеров.ФайлЗагружен(ИмяФайла);  
				
		Если  СтруктураОтвета.Ошибка = Ложь Тогда
			Продолжить;
		КонецЕсли;
		...
		Попытка
			...
			МассивФайловАрхива.Добавить(ИмяФайла);
		...
		МЗ = РегистрыСведений.ЗД_ЗагруженныеФайлыЗамеров.СоздатьМенеджерЗаписи();
		МЗ.ИмяФайла = ИмяФайла;
		МЗ.Ошибка = Ошибка;  
		МЗ.ДатаЗагрузки = ТекущаяДата();		
		МЗ.Попыток = СтруктураОтвета.Попыток + 1;
		МЗ.Записать(Истина);
...
Процедура АрхивацияФайлов()
	
	КаталогЗагрузки = Константы.ЗД_ПапкаФайлов_apdex.Получить(); 
	
	//получить все успешно загруженные файлы
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ЗД_ЗагруженныеФайлыЗамеров.ИмяФайла КАК ИмяФайла
		|ИЗ
		|	РегистрСведений.ЗД_ЗагруженныеФайлыЗамеров КАК ЗД_ЗагруженныеФайлыЗамеров
		|ГДЕ
		|	ЗД_ЗагруженныеФайлыЗамеров.Ошибка = ЛОЖЬ";
	
	ТаблицаФайловОбработанных = Запрос.Выполнить().Выгрузить();	
	ТаблицаФайловОбработанных.Индексы.Добавить("ИмяФайла");

	//поиск и анализ файлов для архива
	ТЗФайл = Новый ТаблицаЗначений;
	ТЗФайл.Колонки.Добавить("ИмяФайла");
	ТЗФайл.Колонки.Добавить("Группировка");
	Для Каждого Файл Из НайтиФайлы(КаталогЗагрузки, "*.XML") Цикл
		
		МассивСтрок = ТаблицаФайловОбработанных.НайтиСтроки(Новый Структура("ИмяФайла",Файл.ПолноеИмя));
		Если МассивСтрок.Количество()>0 Тогда		
			НС = ТЗФайл.Добавить();
			НС.ИмяФайла = Файл.ПолноеИмя;
			НС.Группировка = лев(Файл.ИмяБезРасширения,10);		
		КонецЕсли;
	КонецЦикла;
	
	ТЗ_Группировка = ТЗФайл.Скопировать(,"Группировка");
	ТЗ_Группировка.Свернуть("Группировка");
	
	//архивация
	Для каждого ТекСтрока из ТЗ_Группировка Цикл
		
		МассивСтрок = ТЗФайл.НайтиСтроки(Новый Структура("Группировка",ТекСтрока.Группировка));	
		ИмяАрхивногоФайла = КаталогЗагрузки+"\"+СтрЗаменить(ТекСтрока.Группировка,"-","_")+".zip";//2023-09-07   
		ЗаписьZIP = Новый ЗаписьZipФайла(ИмяАрхивногоФайла,
		,"Регламент:"+ТекущаяДата(),,,);
		Для каждого ТекСтрока из МассивСтрок Цикл
			ЗаписьZIP.Добавить(ТекСтрока.ИмяФайла, РежимСохраненияПутейZIP.НеСохранятьПути);	
		КонецЦикла;  
		
		Попытка
			ЗаписьZIP.Записать();
		Исключение
			ЗаписьЖурналаРегистрации("apdex.архивация файлов",УровеньЖурналаРегистрации.Ошибка,,
			,ТекСтрока.ИмяФайла+" "+ОписаниеОшибки());					
			Продолжить;
		КонецПопытки;  
		//Удаление файлов  	
		Для каждого ТекСтрока из МассивСтрок Цикл
			...
			Исключение
				ЗаписьЖурналаРегистрации("apdex.удаление файлов",УровеньЖурналаРегистрации.Ошибка,,
				,ТекСтрока.ИмяФайла+" "+ОписаниеОшибки());					
			КонецПопытки;			
		КонецЦикла; 
